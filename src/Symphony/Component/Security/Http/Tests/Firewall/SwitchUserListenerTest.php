...
use Symphony\Component\Security\Http\Event\SwitchUserEvent;
use Symphony\Component\Security\Http\Firewall\SwitchUserListener;
...
class SwitchUserListenerTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->userProvider = $this->getMockBuilder('Symphony\Component\Security\Core\User\UserProviderInterface')->getMock();
        $this->userChecker = $this->getMockBuilder('Symphony\Component\Security\Core\User\UserCheckerInterface')->getMock();
        $this->accessDecisionManager = $this->getMockBuilder('Symphony\Component\Security\Core\Authorization\AccessDecisionManagerInterface')->getMock();
        $this->request = new Request();
        $this->event = new GetResponseEvent($this->getMockBuilder('Symphony\Component\HttpKernel\HttpKernelInterface')->getMock(), $this->request, HttpKernelInterface::MASTER_REQUEST);
    }
    ...
    public function testExitUserUpdatesToken()
    {
        ...
        $this->assertInstanceOf('Symphony\Component\HttpFoundation\RedirectResponse', $this->event->getResponse());
    ...
    {
        $originalUser = $this->getMockBuilder('Symphony\Component\Security\Core\User\UserInterface')->getMock();
        $refreshedUser = $this->getMockBuilder('Symphony\Component\Security\Core\User\UserInterface')->getMock();
        $this
            ->userProvider
            ->expects($this->any())
            ->method('refreshUser')
    ...
    public function testSwitchUser()
    {
        ...
        $this->assertSame(array(), $this->request->query->all());
        $this->assertSame('', $this->request->server->get('QUERY_STRING'));
        $this->assertInstanceOf('Symphony\Component\Security\Core\Authentication\Token\UsernamePasswordToken', $this->tokenStorage->getToken());
    }
