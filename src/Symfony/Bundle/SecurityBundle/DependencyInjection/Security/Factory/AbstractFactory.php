...
abstract class AbstractFactory implements SecurityFactoryInterface
{
    ...
    protected function createListener($container, $id, $config, $userProvider)
    {
        ...
        $listener = $this->createChildDefinition($listenerId);
    ...
    protected function createAuthenticationSuccessHandler($container, $id, $config)
    {
        ...
        if (isset($config['success_handler'])) {
            $successHandler = $container->setDefinition($successHandlerId, $this->createChildDefinition('security.authentication.custom_success_handler'));
            ...
            $successHandler = $container->setDefinition($successHandlerId, $this->createChildDefinition('security.authentication.success_handler'));
    ...
    protected function createAuthenticationFailureHandler($container, $id, $config)
    {
        ...
        if (isset($config['failure_handler'])) {
            $failureHandler = $container->setDefinition($id, $this->createChildDefinition('security.authentication.custom_failure_handler'));
            $failureHandler->replaceArgument(0, new Reference($config['failure_handler']));
            $failureHandler->replaceArgument(1, $options);
        } else {
            $failureHandler = $container->setDefinition($id, $this->createChildDefinition('security.authentication.failure_handler'));
