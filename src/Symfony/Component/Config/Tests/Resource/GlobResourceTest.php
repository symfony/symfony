...
class GlobResourceTest extends TestCase
{
    ...
    public function testIterator()
    {
        ...
        self::assertEquals([$file => new \SplFileInfo($file)], $paths);
        self::assertInstanceOf(\SplFileInfo::class, current($paths));
        self::assertSame($dir, $resource->getPrefix());
        ...
        self::assertEquals([$file => $file], $paths);
        self::assertInstanceOf(\SplFileInfo::class, current($paths));
        self::assertSame($dir, $resource->getPrefix());
    }
    ...
    public function testIteratorForExclusionDoesntIterateThroughSubfolders()
    {
        ...
        self::assertArrayHasKey($file, $paths);
    ...
    public function testIteratorSkipsFoldersForGivenExcludedPrefixes()
    {
        ...
        self::assertArrayNotHasKey($file, $paths);
    ...
    public function testIsFreshNonRecursiveDetectsNewFile()
    {
        ...
        self::assertTrue($resource->isFresh(0));
        ...
        touch($dir.'/TmpGlob');
        self::assertFalse($resource->isFresh(0));
    ...
    public function testBraceFallback()
    {
        ...
        self::assertSame($expected, array_keys(iterator_to_array($resource)));
    }
    ...
    public function testUnbalancedBraceFallback()
    {
        ...
        self::assertSame([], array_keys(iterator_to_array($resource)));
    }
    ...
    public function testSerializeUnserialize()
    {
        ...
        self::assertEquals($p->getValue($resource), $p->getValue($newResource));
    }
    ...
    public function testPhar()
    {
        ...
        try {
            ...
            self::assertSame(["phar://some.phar{$s}ProjectWithXsdExtensionInPhar.php", "phar://some.phar{$s}schema{$s}project-1.0.xsd"], $files);

            $resource = new GlobResource("phar://some.phar{$s}ProjectWithXsdExtensionInPhar.php", '', true);
            $files = array_keys(iterator_to_array($resource));
            self::assertSame(["phar://some.phar{$s}ProjectWithXsdExtensionInPhar.php"], $files);
        } finally {
            chdir($cwd);
        }
    }
