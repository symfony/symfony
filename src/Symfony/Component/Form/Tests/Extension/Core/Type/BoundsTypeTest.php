...
class BoundsTypeTest extends BaseTypeTestCase
{
    ...
    public function testABoundReportsItsOwnErrors()
    {
        ...
        // the message of the inner type, not one the range makes up for it
        $this->assertSame('Please enter an integer.', $errors[0]->getMessageTemplate());
    }

    public function testInvalidMessageCanBeOverriddenForBothBounds()
    {
        $form = $this->factory->create(static::TESTED_TYPE, null, [
            'type' => IntegerType::class,
            'options' => ['invalid_message' => 'Please enter a valid age range.'],
        ]);
        $form->submit(['from' => 'not a number', 'to' => 'not a number either']);

        $this->assertCount(1, $fromErrors = $form['from']->getErrors());
        $this->assertSame('Please enter a valid age range.', $fromErrors[0]->getMessageTemplate());
        $this->assertCount(1, $toErrors = $form['to']->getErrors());
        $this->assertSame('Please enter a valid age range.', $toErrors[0]->getMessageTemplate());
    }
