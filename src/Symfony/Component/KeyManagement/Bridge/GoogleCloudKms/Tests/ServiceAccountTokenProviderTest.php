...
class ServiceAccountTokenProviderTest extends TestCase
{
    ...
    public function testAnEcPrivateKeyIsRejected()
    {
        $key = openssl_pkey_new(['private_key_type' => \OPENSSL_KEYTYPE_EC, 'curve_name' => 'prime256v1']);
        if (false === $key || !openssl_pkey_export($key, $pem)) {
            $this->markTestSkipped('This OpenSSL build cannot create an EC key.');
        }

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('must be an RSA private key');
        (new ServiceAccountTokenProvider(new MockHttpClient([]), 'sa@x.iam.gserviceaccount.com', $pem))->getToken();
    }
