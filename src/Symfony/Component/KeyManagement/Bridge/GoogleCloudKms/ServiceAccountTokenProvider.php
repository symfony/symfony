...
final class ServiceAccountTokenProvider implements TokenProviderInterface
{
    ...
    /**
     * Google refuses an assertion whose `exp` is more than one hour after `iat`.
     */
    private const int ASSERTION_LIFETIME_SECONDS = 3600;
    ...
    public static function fromJsonString(HttpClientInterface $client, #[\SensitiveParameter] string $json, string $scope = self::DEFAULT_SCOPE): self
    {
        ...
        return new self(
            ...
            $payload['private_key'],
            $payload['token_uri'] ?? self::DEFAULT_TOKEN_URI,
            $scope,
        );
    }
    ...
    public function getToken(): string
    {
        ...
        try {
            ...
            throw new RuntimeException(\sprintf('Failed to read the Google token endpoint response (HTTP %d) for "%s".', $status, $response->getInfo('url')), 0, $e);
        }

        if ($status >= 400) {
            $description = $payload['error_description'] ?? $payload['error'] ?? null;
            $description = \is_string($description) ? $description : 'unknown error';
    ...
    private function buildAssertion(int $now): string
    {
        ...
        $issuedAt = $now - self::IAT_CLOCK_SKEW_SECONDS;
        ...
        if (\OPENSSL_KEYTYPE_RSA !== (openssl_pkey_get_details($key)['type'] ?? null)) {
            throw new InvalidArgumentException('The service-account private key must be an RSA private key.');
        }
