...
use Symphony\Bridge\Twig\Extension\WorkflowExtension;
use Symphony\Component\Workflow\Definition;
use Symphony\Component\Workflow\Metadata\InMemoryMetadataStore;
use Symphony\Component\Workflow\Registry;
use Symphony\Component\Workflow\SupportStrategy\ClassInstanceSupportStrategy;
use Symphony\Component\Workflow\SupportStrategy\InstanceOfSupportStrategy;
...
class WorkflowExtensionTest extends TestCase
{
    ...
    {
        if (!class_exists(InMemoryMetadataStore::class)) {
            $this->markTestSkipped('This test requires symphony/workflow:4.1.');
        }
        $subject = new \stdClass();
        $subject->marking = array();
