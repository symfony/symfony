...
class CrawlerSelectorExistsTest extends TestCase
{
    public function testConstraint()
    {
        ...
        $this->assertTrue($constraint->evaluate(new DomCrawler('<html><head><title>'), '', true));
        $constraint = new CrawlerSelectorExists('h1');
        $this->assertFalse($constraint->evaluate(new DomCrawler('<html><head><title>'), '', true));

        try {
            $constraint->evaluate(new DomCrawler('<html><head><title>'));
