...
class LdapManagerTest extends LdapTestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->adapter->getConnection()->bind('cn=admin,dc=symphony,dc=com', 'symphony');
    }
    ...
    public function testLdapAddAndRemove()
    {
        ...
        $entry = new Entry('cn=Charles Sarrazin,dc=symphony,dc=com', array(
            'sn' => array('csarrazi'),
            'objectclass' => array(
                'inetOrgPerson',
            ),
    ...
    private function executeSearchQuery($expectedResults = 1)
    {
        $results = $this
            ->adapter
            ->createQuery('dc=symphony,dc=com', '(objectclass=person)')
            ->execute()
        ;
