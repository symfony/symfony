 ...
 * Add the `tls_client_auth` and `self_signed_tls_client_auth` client authentication methods to the `oidc_login` authenticator, which authenticate the client with the certificate it presents in the TLS handshake (RFC 8705 §2) and make the requests carrying it to the `mtls_endpoint_aliases` of the provider (RFC 8705 §5)
