...
abstract class AbstractDescriptorTest extends TestCase
{
    ...
    public function testDescribeDefaults($object, array $options, $fixtureName)
    {
        ...
        if ('json' === $this->getFormat()) {
            self::assertEquals(json_encode(json_decode($expectedDescription), \JSON_PRETTY_PRINT), json_encode(json_decode($describedObject), \JSON_PRETTY_PRINT));
        } else {
    ...
    public function testDescribeOption(OptionsResolver $optionsResolver, array $options, $fixtureName)
    {
        ...
        if ('json' === $this->getFormat()) {
            ...
            self::assertStringMatchesFormat(trim($expectedDescription), trim(str_replace(\PHP_EOL, "\n", $describedObject)));
        }
    }
