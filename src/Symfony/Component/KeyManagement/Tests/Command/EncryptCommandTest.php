...
class EncryptCommandTest extends TestCase
{
    ...
    public function testAMissingKeyIsReportedWithoutATrace()
    {
        $tester = $this->tester(['default' => new OpenSslKms(new InMemoryKeyLoader([]))]);
        $exit = $tester->execute(['key-id' => 'missing', 'value' => 'hello'], ['capture_stderr_separately' => true]);

        $this->assertSame(Command::INVALID, $exit);
        $this->assertStringContainsString('Key "missing" was not found in the KMS.', $tester->getErrorOutput());
    ...
    public static function provideBackendFailures(): iterable
    {
        ...
        yield 'an operation the backend does not support' => [static fn () => new UnsupportedOperationException('Data keys are not supported.'), 'Data keys are not supported.', Command::INVALID];
        yield 'a backend that is down' => [static fn () => new RuntimeException('The backend is down.'), 'The backend is down.', Command::FAILURE];
    }

    public function testTheHelpWarnsThatTheValueArgumentIsVisible()
    {
        $help = new Command(null, new EncryptCommand())->getHelp();

        $this->assertStringContainsString('shell history', $help);
