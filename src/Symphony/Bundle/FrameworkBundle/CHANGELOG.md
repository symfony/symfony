 ...
 * The `RequestDataCollector` class has been deprecated. Use the `Symphony\Component\HttpKernel\DataCollector\RequestDataCollector` class instead.
 ...
 * Deprecated `Symphony\Bundle\FrameworkBundle\Controller\ControllerNameParser`
 ...
 * Removed `PhpStringTokenParser`, use `Symphony\Component\Translation\Extractor\PhpStringTokenParser` instead.
 * Removed `PhpExtractor`, use `Symphony\Component\Translation\Extractor\PhpExtractor` instead.
 ...
 * The `symphony/stopwatch` dependency has been removed, require it via `composer
   require symphony/stopwatch` in your `dev` environment.
 ...
 * Deprecated `TranslationDumperPass`, use 
   `Symphony\Component\Translation\DependencyInjection\TranslationDumperPass` instead
 * Deprecated `TranslationExtractorPass`, use 
   `Symphony\Component\Translation\DependencyInjection\TranslationExtractorPass` instead
 * Deprecated `TranslatorPass`, use 
   `Symphony\Component\Translation\DependencyInjection\TranslatorPass` instead
 ...
 * Added `cache:pool:prune` command to allow manual stale cache item pruning of supported PSR-6 and PSR-16 cache pool
   implementations
 * Deprecated `Symphony\Bundle\FrameworkBundle\Translation\TranslationLoader`, use 
   `Symphony\Component\Translation\Reader\TranslationReader` instead
 ...
 * Not defining the `type` option of the `framework.workflows.*` configuration entries is deprecated.
   The default value will be `state_machine` in Symphony 4.0.
 ...
 * Added `Symphony\Bundle\FrameworkBundle\Controller\AbstractController`. It provides
 ...
 * Deprecated `Symphony\Bundle\FrameworkBundle\DependencyInjection\Compiler\AddConsoleCommandPass`. Use `Symphony\Component\Console\DependencyInjection\AddConsoleCommandPass` instead.
 * Added configurable paths for validation files
 * Deprecated `SerializerPass`, use `Symphony\Component\Serializer\DependencyInjection\SerializerPass` instead
 * Deprecated `FormPass`, use `Symphony\Component\Form\DependencyInjection\FormPass` instead
 * Deprecated `SessionListener`
 * Deprecated `TestSessionListener`
 * Deprecated `Symphony\Bundle\FrameworkBundle\DependencyInjection\Compiler\ConfigCachePass`.
   Use tagged iterator arguments instead.
 * Deprecated `PropertyInfoPass`, use `Symphony\Component\PropertyInfo\DependencyInjection\PropertyInfoPass` instead
 * Deprecated `ControllerArgumentValueResolverPass`. Use
   `Symphony\Component\HttpKernel\DependencyInjection\ControllerArgumentValueResolverPass` instead
 * Deprecated `RoutingResolverPass`, use `Symphony\Component\Routing\DependencyInjection\RoutingResolverPass` instead
 * [BC BREAK] The `server:run`, `server:start`, `server:stop` and
   `server:status` console commands have been moved to a dedicated bundle.
   Require `symphony/web-server-bundle` in your composer.json and register
   `Symphony\Bundle\WebServerBundle\WebServerBundle` in your AppKernel to use them.
 ...
 * Deprecated `AddValidatorInitializersPass`, use
   `Symphony\Component\Validator\DependencyInjection\AddValidatorInitializersPass` instead
 * Deprecated `AddConstraintValidatorsPass`, use
   `Symphony\Component\Validator\DependencyInjection\AddConstraintValidatorsPass` instead
 * Deprecated `ValidateWorkflowsPass`, use
   `Symphony\Component\Workflow\DependencyInjection\ValidateWorkflowsPass` instead
 * Deprecated `ConstraintValidatorFactory`, use
   `Symphony\Component\Validator\ContainerConstraintValidatorFactory` instead.
 * Deprecated `PhpStringTokenParser`, use
   `Symphony\Component\Translation\Extractor\PhpStringTokenParser` instead.
 * Deprecated `PhpExtractor`, use
   `Symphony\Component\Translation\Extractor\PhpExtractor` instead.

3.2.0
-----

 * Removed `doctrine/annotations` from the list of required dependencies in `composer.json`
 * Removed `symphony/security-core` and `symphony/security-csrf` from the list of required dependencies in `composer.json`
 * Removed `symphony/templating` from the list of required dependencies in `composer.json`
 * Removed `symphony/translation` from the list of required dependencies in `composer.json`
 * Removed `symphony/asset` from the list of required dependencies in `composer.json`
 ...
 * Added `Symphony\Component\HttpHernel\DataCollector\RequestDataCollector::onKernelResponse()`
 * Added `Symphony\Bundle\FrameworkBundle\DataCollector\RequestDataCollector`
 ...
 * Deprecated the `RouterApacheDumperCommand` which will be removed in Symphony 3.0.
 ...
 * [BC BREAK] the `Symphony\Bundle\FrameworkBundle\DependencyInjection\Compiler\RegisterKernelListenersPass` was moved
 ...
 * deprecated `Symphony\Bundle\FrameworkBundle\HttpKernel::render()` and `Symphony\Bundle\FrameworkBundle\HttpKernel::forward()`
 * deprecated the `Symphony\Bundle\FrameworkBundle\HttpKernel` class in favor of `Symphony\Component\HttpKernel\DependencyInjection\ContainerAwareHttpKernel`
 ...
 * [BC BREAK] restricted the `Symphony\Bundle\FrameworkBundle\HttpKernel::render()` method to only accept URIs or ControllerReference instances
   * `Symphony\Bundle\FrameworkBundle\HttpKernel::render()` method signature changed and the first argument
     must now be a URI or a ControllerReference instance (the `generateInternalUri()` method was removed)
   * The internal routes (`Resources/config/routing/internal.xml`) have been removed and replaced with a listener (`Symphony\Component\HttpKernel\EventListener\FragmentListener`)
   * The `render` method of the `actions` templating helper signature and arguments changed
 * replaced Symphony\Bundle\FrameworkBundle\Controller\TraceableControllerResolver by Symphony\Component\HttpKernel\Controller\TraceableControllerResolver
 * replaced Symphony\Component\HttpKernel\Debug\ContainerAwareTraceableEventDispatcher by Symphony\Component\HttpKernel\Debug\TraceableEventDispatcher
 ...
 * moved Symphony\Bundle\FrameworkBundle\ContainerAwareEventDispatcher to Symphony\Component\EventDispatcher\ContainerAwareEventDispatcher
 * moved Symphony\Bundle\FrameworkBundle\Debug\TraceableEventDispatcher to Symphony\Component\EventDispatcher\ContainerAwareTraceableEventDispatcher
 ...
 * [BC BREAK] Switched behavior of flash messages to expire flash messages on retrieval
   using Symphony\Component\HttpFoundation\Session\Flash\FlashBag as opposed to on
