 ...
 * Deprecated `Symphony\Component\HttpFoundation\Request::getSession()` when no session has been set. Use `Symphony\Component\HttpFoundation\Request::hasSession()` instead.
 ...
 * Using the `AdvancedUserInterface` is now deprecated. To use the existing
   ...
   `Symphony\Component\Security\Core\User\UserChecker`.
 ...
 * The `SecurityUserValueResolver` class is deprecated, use
   `Symphony\Component\Security\Http\Controller\UserValueResolver` instead.
