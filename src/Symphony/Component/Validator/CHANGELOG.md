 ...
 * not setting the `strict` option of the `Choice` constraint to `true` is
   deprecated and will throw an exception in Symphony 4.0
 * setting the `checkDNS` option of the `Url` constraint to `true` is deprecated in favor of
   the `Url::CHECK_DNS_TYPE_*` constants values and will throw an exception in Symphony 4.0
 ...
 * deprecated `DefaultTranslator` in favor of `Symphony\Component\Translation\IdentityTranslator`
 ...
 * copied the constraints `Optional` and `Required` to the
   `Symphony\Component\Validator\Constraints\` namespace and deprecated the original
   classes.
 ...
 * removed `ClassMetadataFactoryInterface` type hint from `ValidatorBuilderInterface::setMetadataFactory`.
   As of Symphony 2.3, this method will be typed against `MetadataFactoryInterface` instead.
