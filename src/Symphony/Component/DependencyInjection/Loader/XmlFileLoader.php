...
{
    const NS = 'http://symphony.com/schema/dic/services';
            ...
            switch ($arg->getAttribute('type')) {
                ...
                case 'expression':
                    if (!class_exists(Expression::class)) {
                        throw new \LogicException(sprintf('The type="expression" attribute cannot be used without the ExpressionLanguage component. Try running "composer require symphony/expression-language".'));
                    }
    ...
    {
        $schemaLocations = array('http://symphony.com/schema/dic/services' => str_replace('\\', '/', __DIR__.'/schema/dic/services/services-1.0.xsd'));
        ...
        foreach ($schemaLocations as $namespace => $location) {
            ...
            if (0 === stripos($location, 'phar://')) {
                $tmpfile = tempnam(sys_get_temp_dir(), 'symphony');
                if ($tmpfile) {
...
<xsd:schema xmlns="http://symphony.com/schema"
    xmlns:xsd="http://www.w3.org/2001/XMLSchema"
    targetNamespace="http://symphony.com/schema"
...
EOF
    ...
    private function validateExtensions(\DOMDocument $dom, $file)
    {
        foreach ($dom->documentElement->childNodes as $node) {
            if (!$node instanceof \DOMElement || 'http://symphony.com/schema/dic/services' === $node->namespaceURI) {
                continue;
            }
