...
final class DataKeyStore implements RewrappableDataKeyStoreInterface
{
    ...
    private const int MAX_SCOPE_LENGTH = 191;
    ...
    /**
     * @var array<string, true> References of the keys whose row was inserted inside a transaction that has not been seen committed
     */
    private array $uncommitted = [];

    /**
     * @var array<string, int> Instant past which a key is retired, keyed by reference
     */
    ...
    public function rotate(string $scope): DataKeyHandle
    {
        ...
        if ($this->connection->isTransactionActive()) {
            $this->uncommitted[$reference] = true;
        }
    ...
    private function buildSchemaTable(): Table
    {
        return Table::editor()
            ...
            ->addColumn(Column::editor()->setUnquotedName('scope')->setTypeName('string')->setLength(self::MAX_SCOPE_LENGTH)->create())
    ...
    private function rowStillExists(string $reference): bool
    {
        $exists = false !== $this->connection->fetchOne(
            \sprintf('SELECT 1 FROM %s WHERE id = ?', $this->table),
            [$reference],
            [ParameterType::BINARY],
        );

        if ($exists && !$this->connection->isTransactionActive()) {
            unset($this->uncommitted[$reference]);
        }
    ...
    {
        if (self::MAX_SCOPE_LENGTH < $length = \strlen($scope)) {
            throw new InvalidArgumentException(\sprintf('A scope is at most %d bytes long, %d given.', self::MAX_SCOPE_LENGTH, $length));
        }
    }
    ...
    private function isRetired(string $reference): bool
    {
        ...
        return time() >= ($this->retiresAt[$reference] ??= $this->retirementOf($reference));
    }
    ...
    private function retirementOf(string $reference): int
    {
        ...
        // keep the ordering, which can land ahead of the clock read later. A max age of zero
        // retires such a key at once rather than counting it as not yet born.
        return 0 === $this->maxAgeSeconds ? 0 : $uid->getDateTime()->getTimestamp() + $this->maxAgeSeconds;
    }
}
