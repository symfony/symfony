...
use Symfony\Component\Serializer\Tests\Fixtures\Attributes\IgnoreMethodAndPropertyDummy;
...
class AttributeLoaderTest extends TestCase
{
    ...
    public function testIgnoreOnMethodDoesNotIgnorePublicPropertyWithSameName()
    {
        $classMetadata = new ClassMetadata(IgnoreMethodAndPropertyDummy::class);
        $this->loader->loadClassMetadata($classMetadata);

        $attributesMetadata = $classMetadata->getAttributesMetadata();
        $this->assertArrayHasKey('name', $attributesMetadata);
        $this->assertFalse($attributesMetadata['name']->isIgnored());
    }
