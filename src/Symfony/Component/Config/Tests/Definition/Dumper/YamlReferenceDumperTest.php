...
EOL
    ...
    {
        $config = $this->getMockBuilder(ConfigurationInterface::class)->getMock();
        ...
        $config->expects($this->any())->method('getConfigTreeBuilder')->willReturn($builder);
        $dumper = new YamlReferenceDumper();
        $expected = <<<EOL
dump_example:
    scalar_value:         ~
...
EOL;
        $this->assertEquals($expected, $dumper->dump($config));
    }
