                            ...
                            ->validate()
                                ->ifTrue(function ($v) {
                                    foreach ($v as $key => $config) {
                                        if (empty($config['parent']) && !in_array($key, array('apcu', 'doctrine', 'filesystem', 'psr6', 'redis'))) {
                                            return true;
                                        }
                                    }
                                ...
                                ->thenInvalid('Only "apcu", "doctrine", "filesystem", "psr6" or "redis" cache adapter names can have no "parent" configuration key.')
                            ->end()
                        ->end()
                        ->arrayNode('pools')
                            ->useAttributeAsKey('name')
