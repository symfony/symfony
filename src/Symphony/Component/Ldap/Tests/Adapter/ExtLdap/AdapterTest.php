...
use Symphony\Component\Ldap\Adapter\ExtLdap\Adapter;
use Symphony\Component\Ldap\Adapter\ExtLdap\Collection;
use Symphony\Component\Ldap\Adapter\ExtLdap\Query;
use Symphony\Component\Ldap\Entry;
use Symphony\Component\Ldap\Exception\NotBoundException;
...
class AdapterTest extends LdapTestCase
{
    ...
    public function testLdapQuery()
    {
        ...
        $ldap->getConnection()->bind('cn=admin,dc=symphony,dc=com', 'symphony');
        $query = $ldap->createQuery('dc=symphony,dc=com', '(&(objectclass=person)(ou=Maintainers))', array());
        ...
        $this->assertEquals(array('fabpot@symphony.com', 'fabien@potencier.com'), $entry->getAttribute('mail'));
    }
    ...
    public function testLdapQueryScopeBase()
    {
        ...
        $query = $ldap->createQuery('cn=Fabien Potencier,dc=symphony,dc=com', '(objectclass=*)', array(
    ...
    public function testLdapQueryScopeOneLevel()
    {
        ...
        $one_level_result = $ldap->createQuery('ou=Components,dc=symphony,dc=com', '(objectclass=*)', array(
            'scope' => Query::SCOPE_ONE,
        ))->execute();

        $subtree_count = $ldap->createQuery('ou=Components,dc=symphony,dc=com', '(objectclass=*)')->execute()->count();
