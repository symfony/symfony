...
final class BlindIndexListener
{
    ...
    public function onFlush(OnFlushEventArgs $event): void
    {
        ...
        foreach ([$unitOfWork->getScheduledEntityInsertions(), $unitOfWork->getScheduledEntityUpdates()] as $entities) {
            foreach ($entities as $entity) {
                $classMetadata = $manager->getClassMetadata($entity::class);

                if ($this->fill($entity, $classMetadata)) {
     ...
     */
    private function fill(object $entity, ClassMetadata $classMetadata): bool
    {
        $written = false;
    ...
    private function indexedProperties(ClassMetadata $classMetadata): array
    {
        ...
        foreach ($properties as $target) {
            foreach ($target->getAttributes(BlindIndexed::class) as $attribute) {
                ...
                if (!$classMetadata->hasField($target->name)) {
                    throw new \LogicException(\sprintf('The property "%s::$%s" carries "%s" but is not a mapped column, so the tag it holds would never be persisted.', $class, $target->name, BlindIndexed::class));
                }

                $source = $properties[$attribute->property];
                if ((null === $source->getType() || $source->getType()->allowsNull()) && !($target->getType()?->allowsNull() ?? true)) {
