...
class GenericTypePropertyMetadataLoaderTest extends TestCase
{
    public function testReplaceGenerics()
    {
        ...
        $metadata = $loader->load(DummyWithGenerics::class, context: ['original_type' => Type::generic(Type::string(), Type::generic(Type::object(DummyWithGenerics::class), Type::int()))]);
        $this->assertEquals(['foo' => new PropertyMetadata('foo', Type::int())], $metadata);
        ...
        $metadata = $loader->load(DummyWithGenerics::class, context: ['original_type' => Type::intersection(Type::string(), Type::generic(Type::object(DummyWithGenerics::class), Type::int()))]);
        $this->assertEquals(['foo' => new PropertyMetadata('foo', Type::int())], $metadata);
    }
