    ...
    private function addFirewallsSection(ArrayNodeDefinition $rootNode, array $factories): void
    {
        ...
        $firewallNodeBuilder
            ...
            ->scalarNode('re_authentication_entry_point')
                ->info(\sprintf('Service id implementing "%s", asking an already authenticated user to authenticate again when a single attribute it acts on is denied, IS_AUTHENTICATED_RECENTLY and IS_AUTHENTICATED_VERY_RECENTLY for the one oidc_login ships. Defaults to the firewall entry point when that one implements it.', ReAuthenticationEntryPointInterface::class))
            ->end()
