...
namespace Symphony\Bundle\FrameworkBundle\DependencyInjection;

use Doctrine\Common\Annotations\Reader;
use Doctrine\Common\Annotations\AnnotationRegistry;
use Symphony\Bridge\Doctrine\Messenger\DoctrineTransactionMiddleware;
use Symphony\Bridge\Monolog\Processor\DebugProcessor;
...
use Symphony\Bundle\FrameworkBundle\Routing\AnnotatedRouteControllerLoader;
use Symphony\Bundle\FullStack;
use Symphony\Component\Cache\Adapter\AbstractAdapter;
...
use Symphony\Component\DependencyInjection\ParameterBag\ParameterBagInterface;
...
use Symphony\Component\HttpKernel\CacheClearer\CacheClearerInterface;
...
use Symphony\Component\Lock\Store\StoreFactory;
...
use Symphony\Component\Routing\Loader\AnnotationDirectoryLoader;
...
use Symphony\Component\Serializer\Encoder\DecoderInterface;
use Symphony\Component\Serializer\Encoder\EncoderInterface;
...
use Symphony\Component\Serializer\Normalizer\DateIntervalNormalizer;
use Symphony\Component\Serializer\Normalizer\ConstraintViolationListNormalizer;
...
use Symphony\Component\Translation\Command\XliffLintCommand as BaseXliffLintCommand;
use Symphony\Component\Translation\Translator;
use Symphony\Component\Validator\ConstraintValidatorInterface;
use Symphony\Component\Validator\ObjectInitializerInterface;
use Symphony\Component\WebLink\HttpHeaderSerializer;
use Symphony\Component\Workflow;
use Symphony\Component\Yaml\Command\LintCommand as BaseYamlLintCommand;
...
class FrameworkExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if ($this->isConfigEnabled($container, $config['translator']) || $this->isConfigEnabled($container, $config['form']) || $this->isConfigEnabled($container, $config['validation'])) {
            if (!class_exists('Symphony\Component\Translation\Translator') && $this->isConfigEnabled($container, $config['translator'])) {
        ...
        if ($this->isConfigEnabled($container, $config['form'])) {
            if (!class_exists('Symphony\Component\Form\Form')) {
            ...
            if (class_exists('Symphony\Component\Validator\Validation')) {
        ...
        if ($this->isConfigEnabled($container, $config['assets'])) {
            if (!class_exists('Symphony\Component\Asset\Package')) {
        ...
        if ($this->isConfigEnabled($container, $config['templating'])) {
            if (!class_exists('Symphony\Component\Templating\PhpEngine')) {
        ...
        if ($this->isConfigEnabled($container, $config['serializer'])) {
            if (!class_exists('Symphony\Component\Serializer\Serializer')) {
        ...
        $this->addAnnotatedClassesToCompile(array(
            ...
            'Symphony\\Bundle\\FrameworkBundle\\Controller\\AbstractController',
            'Symphony\\Bundle\\FrameworkBundle\\Controller\\Controller',
        ));
    ...
    private function registerTranslatorConfiguration(array $config, ContainerBuilder $container, LoaderInterface $loader)
    {
        ...
        $dirs = array();
        if (class_exists('Symphony\Component\Validator\Validation')) {
            $r = new \ReflectionClass('Symphony\Component\Validator\Validation');

            $dirs[] = dirname($r->getFileName()).'/Resources/translations';
        }
        if (class_exists('Symphony\Component\Form\Form')) {
            $r = new \ReflectionClass('Symphony\Component\Form\Form');

            $dirs[] = dirname($r->getFileName()).'/Resources/translations';
        }
        if (class_exists('Symphony\Component\Security\Core\Exception\AuthenticationException')) {
            $r = new \ReflectionClass('Symphony\Component\Security\Core\Exception\AuthenticationException');
    ...
    private function registerValidationConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
    {
        ...
        if (!$container->getParameter('kernel.debug')) {
            $validatorBuilder->addMethodCall('setMetadataCache', array(new Reference('validator.mapping.cache.symphony')));
        }
    }
    ...
    private function registerValidatorMapping(ContainerBuilder $container, array $config, array &$files)
    {
        ...
        if (interface_exists('Symphony\Component\Form\FormInterface')) {
            $reflClass = new \ReflectionClass('Symphony\Component\Form\FormInterface');
    ...
    {
        if (!class_exists('Symphony\Component\PropertyAccess\PropertyAccessor')) {
            return;
        }
    ...
    private function registerSecurityCsrfConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
    {
        ...
        if (!class_exists('Symphony\Component\Security\Csrf\CsrfToken')) {
            throw new LogicException('CSRF support cannot be enabled as the Security CSRF component is not installed. Try running "composer require symphony/security-csrf".');
        }
    ...
    private function registerSerializerConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
    {
        ...
        if (!class_exists(ClassDiscriminatorFromClassMetadata::class)) {
            $container->removeAlias('Symphony\Component\Serializer\Mapping\ClassDiscriminatorResolverInterface');
        ...
        if (isset($config['enable_annotations']) && $config['enable_annotations']) {
            ...
            $annotationLoader = new Definition(
                'Symphony\Component\Serializer\Mapping\Loader\AnnotationLoader',
        ...
        $fileRecorder = function ($extension, $path) use (&$serializerLoaders) {
            $definition = new Definition(in_array($extension, array('yaml', 'yml')) ? 'Symphony\Component\Serializer\Mapping\Loader\YamlFileLoader' : 'Symphony\Component\Serializer\Mapping\Loader\XmlFileLoader', array($path));
            $definition->setPublic(false);
        ...
        if (!$container->getParameter('kernel.debug')) {
            $cacheMetadataFactory = new Definition(
                ...
                array(
                    ...
                    new Reference('serializer.mapping.cache.symphony'),
                )
            );
    ...
    private function registerPropertyInfoConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
    {
        ...
        if (interface_exists('phpDocumentor\Reflection\DocBlockFactoryInterface')) {
            $definition = $container->register('property_info.php_doc_extractor', 'Symphony\Component\PropertyInfo\Extractor\PhpDocExtractor');
    ...
    private function registerMessengerConfiguration(array $config, ContainerBuilder $container, XmlFileLoader $loader)
    {
        ...
        if ($config['middlewares']['doctrine_transaction']['enabled']) {
            if (!class_exists(DoctrineTransactionMiddleware::class)) {
                throw new LogicException('The Doctrine transaction middleware is only available when the doctrine bridge is installed. Try running "composer require symphony/doctrine-bridge".');
            }
    ...
    {
        return 'http://symphony.com/schema/dic/symphony';
    }
}
