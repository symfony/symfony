...
class WorkflowValidatorTest extends TestCase
{
    ...
    public function testWithArcInFromTooHeavy()
    {
        ...
        $this->expectException(InvalidDefinitionException::class);
        $this->expectExceptionMessage('The marking store of workflow "foo" cannot store many places. But the transition "t1" has an arc from "a" to the transition with a weight equals to 2.');
    ...
    public function testWithArcInToTooHeavy()
    {
        ...
        $this->expectException(InvalidDefinitionException::class);
        $this->expectExceptionMessage('The marking store of workflow "foo" cannot store many places. But the transition "t1" has an arc from the transition to "b" with a weight equals to 2.');
