...
class CachePoolsTest extends AbstractWebTestCase
{
    ...
    private function doTestCachePools($options, $adapterClass)
    {
        ...
        self::assertInstanceOf($adapterClass, $pool1);
        ...
        self::assertInstanceof(TagAwareAdapter::class, $pool4);
        $pool4 = (array) $pool4;
        self::assertSame($pool4[$prefix.'pool'], $pool4[$prefix.'tags'] ?? $pool4['tags']);

        $pool5 = $container->get('cache.pool5');
        self::assertInstanceof(TagAwareAdapter::class, $pool5);
        $pool5 = (array) $pool5;
        self::assertSame($pool2, $pool5[$prefix.'tags'] ?? $pool5['tags']);

        $pool6 = $container->get('cache.pool6');
        self::assertInstanceof(TagAwareAdapter::class, $pool6);
        $pool6 = (array) $pool6;
        self::assertSame($pool4[$prefix.'pool'], $pool6[$prefix.'tags'] ?? $pool6['tags']);

        $pool7 = $container->get('cache.pool7');
        self::assertNotInstanceof(TagAwareAdapter::class, $pool7);
    }
