...
use Symphony\Bridge\PhpUnit\ClockMock;
 ...
 * @internal
 */
class SymphonyTestsListenerTrait
{
    ...
    public function __construct(array $mockedNamespaces = array())
    {
        if (class_exists('PHPUnit_Util_Blacklist')) {
            \PHPUnit_Util_Blacklist::$blacklistedClassNames['\Symphony\Bridge\PhpUnit\Legacy\SymphonyTestsListenerTrait'] = 2;
        } else {
            Blacklist::$blacklistedClassNames['\Symphony\Bridge\PhpUnit\Legacy\SymphonyTestsListenerTrait'] = 2;
        }
    ...
    public function startTestSuite($suite)
    {
        ...
        if (-1 === $this->state) {
            ...
            if ($this->skippedFile = getenv('SYMPHONY_PHPUNIT_SKIPPED_TESTS')) {
                $this->state = 1;
    ...
    public function startTest($test)
    {
        if (-2 < $this->state && ($test instanceof \PHPUnit_Framework_TestCase || $test instanceof TestCase)) {
            ...
            if ($this->willBeIsolated($test)) {
                ...
                putenv('SYMPHONY_DEPRECATIONS_SERIALIZE='.$this->runsInSeparateProcess);
            }
    ...
    public function endTest($test, $time)
    {
        ...
        if ($this->runsInSeparateProcess) {
            ...
            putenv('SYMPHONY_DEPRECATIONS_SERIALIZE');
    ...
    public function handleError($type, $msg, $file, $line, $context = array())
    {
        ...
        // by the isolated test path in \Symphony\Bridge\PhpUnit\Legacy\SymphonyTestsListenerTrait::endTest().
