...
namespace Symphony\Bundle\SecurityBundle\Tests\DataCollector;

use PHPUnit\Framework\TestCase;
use Symphony\Bundle\SecurityBundle\DataCollector\SecurityDataCollector;
...
use Symphony\Component\Security\Core\Role\RoleHierarchy;
...
class SecurityDataCollectorTest extends TestCase
{
    ...
    public function testCollectAuthenticationTokenAndRoles(array $roles, array $normalizedRoles, array $inheritedRoles)
    {
        ...
        $this->assertNull($collector->getImpersonatorUser());
        $this->assertNull($collector->getImpersonationExitPath());
        $this->assertSame('Symphony\Component\Security\Core\Authentication\Token\UsernamePasswordToken', $collector->getTokenClass()->getValue());
        $this->assertTrue($collector->supportsRoleHierarchy());
    ...
    {
        return $this
            ->getMockBuilder('Symphony\Component\HttpFoundation\Request')
            ->disableOriginalConstructor()
            ->getMock();
    }
    ...
    {
        return $this
            ->getMockBuilder('Symphony\Component\HttpFoundation\Response')
            ->disableOriginalConstructor()
            ->getMock();
    }
}
