    ...
    public function compile(Compiler $compiler)
    {
        ...
        if (!$this->hasNode('values')) {
            ...
            $compiler
                ...
                ->write("}\n")
                ->addDebugInfo($this)
                ->write(sprintf('\Symphony\Component\VarDumper\VarDumper::dump($%svars);'."\n", $this->varPrefix));
        } elseif (($values = $this->getNode('values')) && 1 === $values->count()) {
            $compiler
                ->addDebugInfo($this)
                ->write('\Symphony\Component\VarDumper\VarDumper::dump(')
                ->subcompile($values->getNode(0))
                ->raw(");\n");
        } else {
            $compiler
                ->addDebugInfo($this)
                ->write('\Symphony\Component\VarDumper\VarDumper::dump(array('."\n")
                ->indent();
