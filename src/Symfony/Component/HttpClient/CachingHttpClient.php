...
class CachingHttpClient implements HttpClientInterface, ResetInterface
{
    ...
    public function __construct(HttpClientInterface $client, StoreInterface $store, array $defaultOptions = [])
    {
        if (!class_exists(HttpClientKernel::class)) {
            throw new \LogicException(sprintf('Using "%s" requires that the HttpKernel component version 4.4 or higher is installed, try running "composer require symfony/http-kernel:^4.4".', __CLASS__));
        }
