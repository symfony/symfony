...
class SodiumKmsFactoryTest extends TestCase
{
    ...
    {
        $this->assertFalse((new SodiumKmsFactory())->supports(Dsn::fromString('hashicorp-vault-transit://x@host')));
    }
    ...
    public function testCreateOnUnsupportedSchemeThrows()
    {
        ...
        (new SodiumKmsFactory())->create(Dsn::fromString('hashicorp-vault-transit://x@host'));
    }
    ...
    public function testInMemoryDsnRejectsAKeyWhosePlusBecameASpace()
    {
        ...
        (new SodiumKmsFactory())->create(Dsn::fromString('sodium://?keys[app]='.base64_encode(str_repeat("\xfb\xef\xbe", 11))));
    }

    public function testInMemoryDsnAcceptsAPercentEncodedStandardBase64Key()
    {
        $key = str_repeat("\xfb\xef\xbe", 10).'xx';
        $dsn = 'sodium://?keys[app]='.rawurlencode(base64_encode($key));
