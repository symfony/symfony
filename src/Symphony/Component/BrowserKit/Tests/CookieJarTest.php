...
class CookieJarTest extends TestCase
{
    ...
    public function testUpdateFromSetCookie()
    {
        ...
        $this->assertInstanceOf('Symphony\Component\BrowserKit\Cookie', $cookieJar->get('foo'));
        $this->assertInstanceOf('Symphony\Component\BrowserKit\Cookie', $cookieJar->get('bar'));
    ...
    public function testUpdateFromSetCookieWithMultipleCookies()
    {
        ...
        $setCookies = array(sprintf('foo=foo; expires=%s; domain=.symphony.com; path=/, bar=bar; domain=.blog.symphony.com, PHPSESSID=id; expires=%1$s', $date));

        $cookieJar = new CookieJar();
        $cookieJar->updateFromSetCookie($setCookies);

        $fooCookie = $cookieJar->get('foo', '/', '.symphony.com');
        $barCookie = $cookieJar->get('bar', '/', '.blog.symphony.com');
        $phpCookie = $cookieJar->get('PHPSESSID');

        $this->assertInstanceOf('Symphony\Component\BrowserKit\Cookie', $fooCookie);
        $this->assertInstanceOf('Symphony\Component\BrowserKit\Cookie', $barCookie);
        $this->assertInstanceOf('Symphony\Component\BrowserKit\Cookie', $phpCookie);
