 ...
 * Add `ReAuthenticationEntryPointInterface`, started by `ExceptionListener` when a voter that denied an attribute names it in `AuthenticatedVoter::RE_AUTHENTICATION`, as `AuthenticatedVoter` does for `IS_AUTHENTICATED_RECENTLY` and `IS_AUTHENTICATED_VERY_RECENTLY`
