...
namespace Symphony\Component\Yaml\Tests;
...
use Symphony\Component\Yaml\Tag\TaggedValue;
...
EOF;
     ...
     *
     * @see https://github.com/symphony/symphony/issues/7989
     */
...
EOF;
    ...
    {
        $input = <<<'EOF'
foo: !php/object O:30:"Symphony\Component\Yaml\Tests\B":1:{s:1:"b";s:3:"foo";}
bar: 1
EOF;
    ...
    {
        $input = <<<'EOF'
foo: !php/object O:30:"Symphony\Tests\Component\Yaml\B":1:{s:1:"b";s:3:"foo";}
bar: 1
EOF;
...
YAML;
    ...
    /**
     * @expectedException \Symphony\Component\Yaml\Exception\ParseException
     */
    ...
    public function testNonUtf8Exception()
    {
        ...
        foreach ($yamls as $yaml) {
            try {
                ...
                $this->assertInstanceOf('Symphony\Component\Yaml\Exception\ParseException', $e, 'charsets other than UTF-8 are rejected.');
            }
        }
    }
...
EOT;
    ...
    {
        if (method_exists($this, 'expectException')) {
            $this->expectException('\Symphony\Component\Yaml\Exception\ParseException');
            $this->expectExceptionMessage(sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        } else {
            $this->setExpectedException('\Symphony\Component\Yaml\Exception\ParseException', sprintf('Unexpected characters near "," at line %d (near "bar: "123",").', $lineNumber));
        }
...
YAML;
    ...
    public function testPhpConstantTagMappingKey()
    {
        $yaml = <<<YAML
transitions:
    !php/const 'Symphony\Component\Yaml\Tests\B::FOO':
        from:
            - !php/const 'Symphony\Component\Yaml\Tests\B::BAR'
        to: !php/const 'Symphony\Component\Yaml\Tests\B::BAZ'
YAML;
        $expected = array(
            'transitions' => array(
                'foo' => array(
