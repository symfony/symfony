...
class AwsKmsTest extends TestCase
{
    ...
    public static function provideUnparsableSuccessBodies(): iterable
    {
        yield 'html' => ['<html>'];
        yield 'empty' => [''];
        yield 'text' => ['nope'];
    ...
    #[DataProvider('provideUnparsableSuccessBodies')]
    public function testEncryptOnAnUnparsableSuccessBodyIsARuntimeException(string $body)
    {
        $kms = self::kmsOverHttp(new MockResponse($body, ['http_code' => 200]));

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('AWS KMS encrypt failed');
        $kms->encrypt('alias/app-key', 'hello');
    }

    #[DataProvider('provideUnparsableSuccessBodies')]
    public function testDecryptOnAnUnparsableSuccessBodyIsARuntimeException(string $body)
    {
        $kms = self::kmsOverHttp(new MockResponse($body, ['http_code' => 200]));

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('AWS KMS decrypt failed');
        $kms->decrypt(new Ciphertext('blob', 'alias/app-key'));
    }

    #[DataProvider('provideUnparsableSuccessBodies')]
    public function testGenerateDataKeyOnAnUnparsableSuccessBodyIsARuntimeException(string $body)
    {
        $kms = self::kmsOverHttp(new MockResponse($body, ['http_code' => 200]));

        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('AWS KMS GenerateDataKey failed');
        $kms->generateDataKey('alias/app-key');
    }

    public function testTransportErrorWhileReadingTheBodyIsARuntimeException()
    {
        $kms = self::kmsOverHttp(new MockResponse((static function (): \Generator {
            yield '{"Plain';
            yield '';
        })(), ['http_code' => 200]));
    ...
    public function testTransportErrorWhileReadingAnErrorBodyIsARuntimeException()
    {
        $kms = self::kmsOverHttp(new MockResponse((static function (): \Generator {
            yield '{"__type';
            yield '';
    ...
    private static function kmsOverHttp(MockResponse $response): AwsKms
    {
        return new AwsKms(new KmsClient(Configuration::create(['region' => 'eu-west-1']), new NullProvider(), new MockHttpClient($response)));
    }
