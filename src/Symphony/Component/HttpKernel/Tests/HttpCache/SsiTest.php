...
class SsiTest extends TestCase
{
    ...
    public function testAddSurrogateSsiCapability()
    {
        ...
        $this->assertEquals('symphony="SSI/1.0"', $request->headers->get('Surrogate-Capability'));

        $ssi->addSurrogateCapability($request);
        $this->assertEquals('symphony="SSI/1.0", symphony="SSI/1.0"', $request->headers->get('Surrogate-Capability'));
    }
