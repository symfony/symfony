 ...
 * round trip it made, so adding the three would report more time than the request spent. The
 * decorators mark where a call starts through {@see enter()}, which is what tells an outermost call
 * from a nested one.
 *
...
class KeyManagementDataCollector extends DataCollector implements LateDataCollectorInterface
{
    ...
    /**
     * Number of traced calls in progress.
     */
    private int $depth = 0;

    /**
     * When the outermost call in progress started, before which nothing can be nested anymore.
     */
    ...
    /**
     * @param list<string> $clients Names of the configured KMS clients, so that the panel says what is wired even when a request used none of it
     */
     ...
     *
     * Wall time and failures are charged to the outermost call: the envelope operation that failed
     * because the KMS refused the data key is one failure, and its duration already covers the
     * round trip.
     *
    ...
    public function collectCall(array $call): void
    {
        ...
        $call['outermost'] = 0 === ($this->depth = max(0, $this->depth - 1));
        ...
        $this->totals['errors'] += $call['outermost'] && null !== $call['error'] ? 1 : 0;
        $this->totals['kms_time'] += self::LAYER_KMS === $call['layer'] ? $call['time'] : 0.0;
        $this->totals['time'] += $call['outermost'] ? $call['time'] : 0.0;
    ...
    {
        $group = &$this->services[$call['layer']][$call['service']];
        $group ??= ['backend' => $call['backend']] + self::newGroup();
     ...
     * @param array<string, mixed> $call
     */
    private function fold(array &$group, array $call): void
    {
        ++$group['ops'];
        ...
        $this->accumulate($group['segments'], $group['time'], $call);
    }
     ...
     *
     * What is kept is what a call still in progress could swallow: the segments that started under
     * the outermost one. An outermost call leaves nothing behind, so a group holds as many segments
     * as one operation nests calls under it, not as many as the request made.
     *
     * @param list<array{start: float, end: float}> $segments
     * @param array<string, mixed>                  $call
     */
    private function accumulate(array &$segments, float &$total, array $call): void
    {
        $start = $call['start'];
        $end = $start + $call['time'];
        ...
        while ($segments && $segments[0]['end'] <= $this->floor) {
    ...
    private function identify(array $call): array
    {
        if (null !== $call['key']) {
            ...
            if (null !== $call['reference']) {
                $this->scopeOf[$call['reference']] = self::printable($call['key']);
            }
