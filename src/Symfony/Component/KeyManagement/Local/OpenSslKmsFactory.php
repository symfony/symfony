 ...
 *     keys are inlined in the DSN, base64url-encoded; a standard base64 key
 *     must be percent-encoded (`+` as `%2B`) since the query string reads
 *     `+` as a space.
 ...
 * `symfony/flysystem-key-management` under the `openssl+fly://` scheme.
 *
...
final class OpenSslKmsFactory implements KmsFactoryInterface
{
    ...
    private function buildKeyLoader(#[\SensitiveParameter] Dsn $dsn): KeyLoaderInterface
    {
