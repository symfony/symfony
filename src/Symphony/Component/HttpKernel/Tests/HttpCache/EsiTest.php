...
class EsiTest extends TestCase
{
    ...
    public function testAddSurrogateEsiCapability()
    {
        ...
        $this->assertEquals('symphony="ESI/1.0"', $request->headers->get('Surrogate-Capability'));

        $esi->addSurrogateCapability($request);
        $this->assertEquals('symphony="ESI/1.0", symphony="ESI/1.0"', $request->headers->get('Surrogate-Capability'));
    }
    ...
    {
        $cache = $this->getMockBuilder('Symphony\Component\HttpKernel\HttpCache\HttpCache')->setMethods(array('getRequest', 'handle'))->disableOriginalConstructor()->getMock();
