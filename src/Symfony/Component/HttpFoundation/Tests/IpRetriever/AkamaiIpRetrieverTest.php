...
class AkamaiIpRetrieverTest extends \PHPUnit_Framework_TestCase
{
    ...
    private function getRequestInstanceForClientIpsTrueClientIpTests($remoteAddr, $httpTrueClientIp)
    {
        $request = new Request();

        $server = array('REMOTE_ADDR' => $remoteAddr);

        if (null !== $httpTrueClientIp) {
            $server['HTTP_TRUE_CLIENT_IP'] = $httpTrueClientIp;
        }
    ...
    /**
     * @dataProvider testGetClientIpsSpecialHeadersProvider
     */
    ...
    public function testGetClientIpsSpecialHeadersProvider()
    {
