...
/**
 * XliffRawLoader loads translations from XLIFF string.
 *
 * @author Fabien Potencier <fabien@symfony.com>
 */
class XliffRawLoader implements LoaderInterface
{
    /**
     * {@inheritdoc}
     */
    public function load($resource, string $locale, string $domain = 'messages')
    {
        try {
            ...
            throw new InvalidResourceException(sprintf('This is not a XLIFF string "%s".', $resource));
        }
    ...
    {
        $xliffVersion = XliffUtils::getVersionNumber($resource);
        if ($errors = XliffUtils::validateSchema($resource)) {
        ...
        if ('1.2' === $xliffVersion) {
            $this->extractXliff1($resource, $catalogue, $domain);
        }

        if ('2.0' === $xliffVersion) {
            $this->extractXliff2($resource, $catalogue, $domain);
        }
    }
