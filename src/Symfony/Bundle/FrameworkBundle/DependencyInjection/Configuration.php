        ...
        $rootNode
            ->children()
                ->arrayNode('templating')
                    ...
                    ->beforeNormalization()
                        ->ifTrue(function($v) { return isset($v['assets_version']); })
                        ->then(function($v) { $v[isset($v['package']) ? 'package' : 'packages']['default']['version'] = $v['assets_version']; unset($v['assets_version']); return $v; })
                    ->end()
                    ->beforeNormalization()
                        ->ifTrue(function($v) { return isset($v['assets_base_url']); })
                        ->then(function($v) { $v['package']['default']['base_urls'] = $v['assets_base_url']; unset($v['assets_base_url']); return $v; })
                    ->end()
                    ->beforeNormalization()
                        ->ifTrue(function($v) { return isset($v['assets_base_urls']); })
                        ->then(function($v) { $v['packages']['default']['base_urls'] = $v['assets_base_urls']; unset($v['assets_base_urls']); return $v; })
                    ->end()
                    ->beforeNormalization()
                        ->ifTrue(function($v) { return isset($v['assets_version_format']); })
                        ->then(function($v) { $v['packages']['default']['version_format'] = $v['assets_version_format']; unset($v['assets_version_format']); return $v; })
                    ->end()
                    ...
                    ->children()
                        ->arrayNode('packages')
                            ...
                            ->prototype('array')
                                ...
                                ->children()
                                    ->arrayNode('base_urls')
                                        // turn a single string into an array
                                        ->beforeNormalization()
                                            ->ifTrue(function($v) { return !is_array($v); })
                                            ->then(function($v) { return array($v); })
                                        ->end()
                                        // organize xml urls
                                        ->beforeNormalization()
                                            ->ifTrue(function($v) { if (!isset($v[0])) return false; foreach ($v as $url) if (is_array($url)) return true; })
                                            ->then(function($v) { $urls = array(); foreach ($v as $url) $urls[is_array($url) && isset($url['secure']) && $url['secure'] ? 'ssl' : 'http'][] = $url; return $urls; })
                                        ->end()
                                        // organize other base urls
                                        ->beforeNormalization()
                                            ->ifTrue(function($v) { return !isset($v['http']) && !isset($v['ssl']); })
                                            ->then(function($v) { return array('http' => $v, 'ssl' => $v); })
                                        ->end()
                                        ->children()
                                            ->arrayNode('http')
                                                // turn a single string into an array
