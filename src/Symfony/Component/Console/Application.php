...
class Application implements ResetInterface
{
    ...
    protected function doRunCommand(Command $command, InputInterface $input, OutputInterface $output): int
    {
        ...
        // bind before getAlarmInterval() and the console.command event, so the method and listeners have access to input options/arguments
        try {
                ...
                foreach ($this->signalsToDispatchEvent as $signal) {
                    ...
                    $this->signalRegistry->register($signal, function ($signal) use ($signalEvent, $alarmEvent, $command, $commandSignals, $input, $output) {
                        ...
                        if (\SIGALRM === $signal && $command instanceof AlarmableCommandInterface) {
                            ...
                            $this->signalRegistry->scheduleAlarm($command->getAlarmInterval($input));
                        }
