...
use Symphony\Component\BrowserKit\Exception\BadMethodCallException;
...
abstract class Client
{
     ...
     *
     * @throws \RuntimeException When Symphony Process Component is not installed
     */
    public function insulate($insulated = true)
    {
        if ($insulated && !class_exists('Symphony\\Component\\Process\\Process')) {
            throw new \RuntimeException('Unable to isolate requests as the Symphony Process Component is not installed.');
        }
    ...
    public function getCrawler()
    {
        if (null === $this->crawler) {
            @trigger_error(sprintf('Calling the "%s()" method before the "request()" one is deprecated since Symphony 4.1 and will throw an exception in 5.0.', __METHOD__), E_USER_DEPRECATED);
    ...
    protected function doRequestInProcess($request)
    {
        ...
        putenv('SYMPHONY_DEPRECATIONS_SERIALIZE='.$deprecationsFile);
        $_ENV['SYMPHONY_DEPRECATIONS_SERIALIZE'] = $deprecationsFile;
    ...
    {
        if (!class_exists('Symphony\Component\DomCrawler\Crawler')) {
            return;
        }
