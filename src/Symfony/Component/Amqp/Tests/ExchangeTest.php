...
class ExchangeTest extends TestCase
{
    ...
    {
        yield array('exchange_name=test_ex.default', 'test_ex.default', \AMQP_EX_TYPE_DIRECT, \AMQP_DURABLE);
        yield array('exchange_name=test_ex.fanout_durable&type=fanout&flags=2', 'test_ex.fanout_durable', \AMQP_EX_TYPE_FANOUT, \AMQP_DURABLE);
    }

    /**
     * @dataProvider getUri
     */
    public function testCreateFromUri(string $qsa, string $name, string $type, int $flags)
    {
        $exchange = Exchange::createFromUri(getenv('AMQP_DSN').'?'.$qsa);
    ...
    {
        Exchange::createFromUri(getenv('AMQP_DSN').'/?type=fanout');
    }
    ...
    public function testPublish()
    {
        ...
        $msg = $this->assertNextMessageBody($message, $name);
