 ...
 * removed the ability to use `FormDataCollector` without the `symphony/var-dumper` component
 ...
 * added an option to ``Symphony\Component\Form\FormRendererEngineInterface::setTheme()`` and
   ``Symphony\Component\Form\FormRendererInterface::setTheme()`` to disable usage of default themes when rendering a form
 ...
 * added `Symphony\Component\Form\FormErrorIterator::findByCodes()`
 * added `getTypedExtensions`, `getTypes`, and `getTypeGuessers` to `Symphony\Component\Form\Test\FormIntegrationTestCase`
 * added `FormPass`
 ...
 * Using callable strings as choice options in `ChoiceType` has been deprecated
   and will be used as `PropertyPath` instead of callable in Symphony 4.0.
 ...
 * added new ChoiceList interface and implementations in the Symphony\Component\Form\ChoiceList namespace
 * added new ChoiceView in the Symphony\Component\Form\ChoiceList\View namespace
 ...
 * [BC BREAK] added two optional parameters to FormInterface::getErrors() and
   changed the method to return a Symphony\Component\Form\FormErrorIterator
 ...
 * deprecated FormPerformanceTestCase and FormIntegrationTestCase in the Symphony\Component\Form\Tests namespace and moved them to the Symphony\Component\Form\Test namespace
 * deprecated TypeTestCase in the Symphony\Component\Form\Tests\Extension\Core\Type namespace and moved it to the Symphony\Component\Form\Test namespace
 ...
 * the PropertyPath class and related classes were moved to a dedicated
   ...
   can now be found in Symphony\Component\PropertyAccess\StringUtil. The methods
