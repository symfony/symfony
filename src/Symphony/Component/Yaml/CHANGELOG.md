 ...
 * Starting an unquoted string with a question mark followed by a space is
   deprecated and will throw a `ParseException` in Symphony 4.0.

 * Deprecated support for implicitly parsing non-string mapping keys as strings.
   Mapping keys that are no strings will lead to a `ParseException` in Symphony
 ...
 * Omitting the key of a mapping is deprecated and will throw a `ParseException` in Symphony 4.0.
 ...
 * Mappings with a colon (`:`) that is not followed by a whitespace are deprecated
   ...
   Symphony 4.0 (e.g. `foo:bar` must be `foo: bar`).
 ...
 * Support for silently ignoring duplicate mapping keys in YAML has been
   deprecated and will lead to a `ParseException` in Symphony 4.0.
