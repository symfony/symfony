...
use Symphony\Component\Security\Http\RememberMe\TokenBasedRememberMeServices;
...
class TokenBasedRememberMeServicesTest extends TestCase
{
    ...
    public function testAutoLogin($username)
    {
        ...
        $this->assertInstanceOf('Symphony\Component\Security\Core\Authentication\Token\RememberMeToken', $returnedToken);
    ...
    {
        $provider = $this->getMockBuilder('Symphony\Component\Security\Core\User\UserProviderInterface')->getMock();
        $provider
            ->expects($this->any())
