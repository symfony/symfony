...
class FactoryRegistryTest extends TestCase
{
    ...
    public function testTheDsnIsASensitiveParameter()
    {
        foreach ([KmsFactoryInterface::class, FactoryRegistry::class] as $class) {
            foreach (['supports', 'create'] as $method) {
                $parameter = new \ReflectionParameter([$class, $method], 'dsn');

                $this->assertNotEmpty($parameter->getAttributes(\SensitiveParameter::class), \sprintf('%s::%s() must redact its DSN.', $class, $method));
            }
        }
    }
