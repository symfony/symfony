...
class CollectionTypeTest extends BaseTypeTestCase
{
    ...
    #[DataProvider('provideKeepAsList')]
    public function testEntryNameIgnoresThePropertyPathOfTheEntryOptions(bool $keepAsList)
    {
        $form = $this->factory->create(static::TESTED_TYPE, [$coffee = new Product(2, 'Coffee')], [
            'entry_type' => ProductType::class,
            'entry_name' => 'id',
            'entry_options' => ['property_path' => null],
            'allow_add' => true,
            'keep_as_list' => $keepAsList,
        ]);

        $this->assertSame($coffee, $form[2]->getData());

        $form->submit([2 => ['name' => 'Espresso'], 'new_1' => ['name' => 'Tea']]);
        ...
        $this->assertSame($coffee, $data[0]);
        $this->assertSame('Espresso', $coffee->name);
        $this->assertSame('Tea', $data[1]->name);
    }

    public static function provideKeepAsList(): iterable
    {
        yield [false];
        yield [true];
    }
