...
class EncryptedType extends Type
{
    ...
    public function convertToDatabaseValue(#[\SensitiveParameter] mixed $value, AbstractPlatform $platform): mixed
    {
        ...
        if (\is_float($plaintext)) {
            if (!is_finite($plaintext)) {
                throw new InvalidArgumentException('The value to encrypt is a float that is not finite, which cannot be stored as text and read back.');
            }

            // A cast to string rounds a float to the "precision" ini setting, which is how many
            // digits a human wants to see, not how many the value has.
