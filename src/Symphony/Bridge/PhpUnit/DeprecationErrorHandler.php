...
class DeprecationErrorHandler
{
    ...
    public static function register($mode = 0)
    {
        ...
        $getMode = function () use ($mode) {
            ...
            if (false === $mode) {
                $mode = getenv('SYMPHONY_DEPRECATIONS_HELPER');
            }
        ...
        $deprecationHandler = function ($type, $msg, $file, $line, $context = array()) use (&$deprecations, $getMode, $UtilPrefix, $inVendors) {
            ...
            if (isset($trace[$i]['object']) || isset($trace[$i]['class'])) {
                if (isset($trace[$i]['class']) && in_array($trace[$i]['class'], array('Symphony\Bridge\PhpUnit\SymphonyTestsListener', 'Symphony\Bridge\PhpUnit\Legacy\SymphonyTestsListener'), true)) {
                    $parsedMsg = unserialize($msg);
                    $msg = $parsedMsg['deprecation'];
                    $class = $parsedMsg['class'];
                    $method = $parsedMsg['method'];
                    // If the deprecation has been triggered via
                    // \Symphony\Bridge\PhpUnit\Legacy\SymphonyTestsListenerTrait::endTest()
