...
abstract class AbstractUriElement
{
     ...
     */
    public function __construct(mixed $node, ?string $currentUri = null, ?string $method = 'GET')
    {
        if (!$node instanceof \DOMElement && !$node instanceof \DOM\Element) {
            throw new \InvalidArgumentException(sprintf('The node should be an instance of \DOMElement or \DOM\Element, "%s" given.', get_debug_type($node)));
        }
