...
final class FlysystemKeyLoader implements KeyLoaderInterface
{
    ...
    {
        // Flysystem turns a backslash into a slash before it resolves the parent segments, so one
        // has to be refused before the segments are looked at.
        foreach (explode('/', $keyId) as $segment) {
            if ('' === $segment || '.' === $segment || '..' === $segment || str_contains($segment, "\0") || str_contains($segment, '\\')) {
