...
class ReflectionExtractorTest extends TestCase
{
    ...
    public function testGetProperties()
    {
        ...
        self::assertNull($this->extractor->getProperties('Symfony\Component\PropertyInfo\Tests\Fixtures\NoProperties'));
    }
    ...
    {
        self::assertEquals($type, $this->extractor->getTypes('Symfony\Component\PropertyInfo\Tests\Fixtures\Php71Dummy', $property, []));
    }
    ...
    {
        self::assertEquals($type, $this->extractor->getTypes('Symfony\Component\PropertyInfo\Tests\Fixtures\Php80Dummy', $property, []));
    }
    ...
    {
        self::assertEquals($type, $this->extractor->getTypes('Symfony\Component\PropertyInfo\Tests\Fixtures\Php81Dummy', $property, []));
    }
    ...
    {
        self::assertFalse($this->extractor->isWritable(Php81Dummy::class, 'foo'));
    }
    ...
    {
        self::assertEquals($type, $this->extractor->getTypes('Symfony\Component\PropertyInfo\Tests\Fixtures\Php82Dummy', $property, []));
    }
    ...
    {
        self::assertEquals($type, $this->extractor->getTypes(DefaultValue::class, $property, []));
    }
    ...
    {
        self::assertSame($expected, $this->extractor->isReadable('Symfony\Component\PropertyInfo\Tests\Fixtures\Dummy', $property, []));
    }
    ...
    {
        self::assertSame($expected, $this->extractor->isWritable(Dummy::class, $property, []));
    }
    ...
    {
        self::assertTrue($this->extractor->isWritable(AdderRemoverDummy::class, 'analyses'));
        self::assertTrue($this->extractor->isWritable(AdderRemoverDummy::class, 'feet'));
        self::assertEquals(['analyses', 'feet'], $this->extractor->getProperties(AdderRemoverDummy::class));
    }
    ...
    public function testPrivatePropertyExtractor()
    {
        ...
        self::assertTrue($privateExtractor->isReadable(Dummy::class, 'bar'));
        self::assertTrue($privateExtractor->isReadable(Dummy::class, 'baz'));
        ...
        self::assertNotContains('bar', $properties);
        self::assertContains('baz', $properties);

        self::assertFalse($protectedExtractor->isReadable(Dummy::class, 'bar'));
        self::assertTrue($protectedExtractor->isReadable(Dummy::class, 'baz'));
    }
    ...
    {
        self::assertSame($expected, $this->extractor->isInitializable($class, $property));
    }
    ...
    public function testExtractTypeConstructor(string $class, string $property, array $type = null)
    {
        ...
        self::assertEquals($type, $this->extractor->getTypes($class, $property, []));
        self::assertEquals($type, $this->extractor->getTypes($class, $property, ['enable_constructor_extraction' => true]));
        self::assertNull($this->extractor->getTypes($class, $property, ['enable_constructor_extraction' => false]));
    }
    ...
    public function testNullOnPrivateProtectedAccessor()
    {
        ...
        self::assertNull($barAcessor);
        self::assertEquals(PropertyWriteInfo::TYPE_NONE, $barMutator->getType());
        self::assertNull($bazAcessor);
        self::assertEquals(PropertyWriteInfo::TYPE_NONE, $bazMutator->getType());
    }
    ...
    {
        self::assertEquals([new Type(Type::BUILTIN_TYPE_OBJECT, false, Dummy::class)], $this->extractor->getTypes(Php74Dummy::class, 'dummy'));
        self::assertEquals([new Type(Type::BUILTIN_TYPE_BOOL, true)], $this->extractor->getTypes(Php74Dummy::class, 'nullableBoolProp'));
        self::assertEquals([new Type(Type::BUILTIN_TYPE_ARRAY, false, null, true, new Type(Type::BUILTIN_TYPE_INT), new Type(Type::BUILTIN_TYPE_STRING))], $this->extractor->getTypes(Php74Dummy::class, 'stringCollection'));
        self::assertEquals([new Type(Type::BUILTIN_TYPE_INT, true)], $this->extractor->getTypes(Php74Dummy::class, 'nullableWithDefault'));
        self::assertEquals([new Type(Type::BUILTIN_TYPE_ARRAY, false, null, true)], $this->extractor->getTypes(Php74Dummy::class, 'collection'));
    }
    ...
    public function testGetReadAccessor($class, $property, $found, $type, $name, $visibility, $static)
    {
        ...
        if (!$found) {
            self::assertNull($readAcessor);

            return;
        }

        self::assertNotNull($readAcessor);
        self::assertSame($type, $readAcessor->getType());
        self::assertSame($name, $readAcessor->getName());
        self::assertSame($visibility, $readAcessor->getVisibility());
        self::assertSame($static, $readAcessor->isStatic());
    }
    ...
    public function testGetWriteMutator($class, $property, $allowConstruct, $found, $type, $name, $addName, $removeName, $visibility, $static)
    {
        ...
        if (!$found) {
            self::assertEquals(PropertyWriteInfo::TYPE_NONE, $writeMutator->getType());
        ...
        if (PropertyWriteInfo::TYPE_PROPERTY === $writeMutator->getType()) {
            self::assertSame($name, $writeMutator->getName());
            self::assertSame($visibility, $writeMutator->getVisibility());
            self::assertSame($static, $writeMutator->isStatic());
        }
