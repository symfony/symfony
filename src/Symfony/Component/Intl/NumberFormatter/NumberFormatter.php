...
abstract class NumberFormatter
{
    ...
    {
        if ($value > self::INT32_MAX || $value < -self::INT32_MAX - 1) {
            return false;
        }
    ...
    {
        if ($value > self::INT64_MAX || $value < -self::INT64_MAX - 1) {
            return false;
        }

        if (\PHP_INT_SIZE !== 8 && ($value > self::INT32_MAX || $value < -self::INT32_MAX - 1)) {
            return (float) $value;
        }
