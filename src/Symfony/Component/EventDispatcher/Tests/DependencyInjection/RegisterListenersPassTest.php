...
class RegisterListenersPassTest extends TestCase
{
    ...
    public function testAServiceIdOutranksTheClassOfAnotherListener()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('impl', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent']);
        $container->register(InvokableListenerService::class, ListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent']);
        $container->register('a', ListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'before' => InvokableListenerService::class]);

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['event', 'impl', 0], ['event', 'a', 0], ['event', InvokableListenerService::class, 0]], $this->getListenerCalls($container));
    }
    ...
    public function testSeveralRegistrationsOfTheSameServiceKeepTheirOwnPlace()
    {
        ...
        $expectedCalls = [['event', 'subscriber', 100], ['event', 'subscriber', -100], ['event', 'a', -200], ['event', 'b', -200]];
    ...
    public function testAConstraintCanTargetASingleMethodOfAListener()
    {
        ...
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'after' => 'sub::onLate']);

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['event', 'sub', 100], ['event', 'sub', -100], ['event', 'a', -100]], $this->getListenerCalls($container));
    }
    ...
    public function testAMethodTargetCanUseTheClassName()
    {
        ...
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('sub', MultiPrioritySubscriber::class)->addTag('kernel.event_subscriber');
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'after' => MultiPrioritySubscriber::class.'::onLate']);
    ...
    public function testAnExplicitPriorityIsNeverRewritten()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'foo', 'method' => 'onEvent', 'priority' => 10]);
        $container->register('b', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'foo', 'method' => 'onEvent', 'priority' => 10, 'before' => 'a']);

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['foo', 'b', 10], ['foo', 'a', 10]], $this->getListenerCalls($container));
    }
    ...
    public function testASubscriberWithoutPriorityIsPlacedByTheConstraintsOfOthers()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 10, 'after' => 'subscriber']);
        $container->register('subscriber', SubscriberService::class)->addTag('kernel.event_subscriber');

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['event', 'subscriber', 10], ['event', 'a', 10]], $this->getListenerCalls($container));
    }

    public function testTheAttributeWithoutPriorityLeavesTheListenerFree()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'foo', 'method' => 'onEvent', 'priority' => 10]);
        $container->register('b', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'foo', 'method' => 'onEvent', 'priority' => null, 'before' => 'a']);
    ...
    public function testAContradictionNamesTheTargetedListener()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('sub', MultiPrioritySubscriber::class)->addTag('kernel.event_subscriber');
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 0, 'before' => 'sub::onEarly']);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('the priority of "a" (0) contradicts its "before" constraint on "sub" (100)');

        (new RegisterListenersPass())->process($container);
    }

    public function testAContradictionNamesTheTargetedMethodWhenItIsNotTheFirstOne()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('sub', MultiPrioritySubscriber::class)->addTag('kernel.event_subscriber');
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 10, 'after' => 'sub::onLate']);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('the priority of "a" (10) contradicts its "after" constraint on "sub::onLate" (-100): lower it to -100 or less');

        (new RegisterListenersPass())->process($container);
    }

    public function testASubscriberCanDeclareConstraintsWithNamedKeys()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 10]);
        $container->register('sub', DeclaringSubscriber::class)->addTag('kernel.event_subscriber');

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['event', 'sub', 10], ['event', 'a', 10]], $this->getListenerCalls($container));
    }

    public function testASubscriberCanDeclareConstraintsInTheListForm()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 0]);
        $container->register('b', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => -5]);
        $container->register('sub', DeclaringListSubscriber::class)->addTag('kernel.event_subscriber');

        (new RegisterListenersPass())->process($container);

        $this->assertSame([['event', 'sub', 100], ['event', 'a', 0], ['event', 'b', -5], ['event', 'sub', -5]], $this->getListenerCalls($container));
    }

    public function testASubscriberConstraintContradictingItsPriorityIsReported()
    {
        $container = new ContainerBuilder();
        $container->register('event_dispatcher', \stdClass::class);
        $container->register('a', InvokableListenerService::class)->addTag('kernel.event_listener', ['event' => 'event', 'method' => 'onEvent', 'priority' => 10]);
        $container->register('sub', PinnedDeclaringSubscriber::class)->addTag('kernel.event_subscriber');

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('the priority of "sub" (0) contradicts its "before" constraint on "a" (10)');
...
final class DeclaringSubscriber implements EventSubscriberInterface
{
    public static function getSubscribedEvents(): array
    {
        return ['event' => ['method' => 'onEvent', 'before' => 'a']];
    }
...
final class DeclaringListSubscriber implements EventSubscriberInterface
{
    public static function getSubscribedEvents(): array
    {
        return ['event' => [
            ['onEarly', 100],
            ['method' => 'onLate', 'after' => ['a', 'b::onEvent']],
        ]];
    }
...
final class PinnedDeclaringSubscriber implements EventSubscriberInterface
{
    public static function getSubscribedEvents(): array
    {
        return ['event' => ['method' => 'onEvent', 'priority' => 0, 'before' => 'a']];
    }
