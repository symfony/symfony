...
class PdoTagAwareAdapter extends AbstractTagAwareAdapter implements PruneableInterface
{
     ...
     *
     * Both, cache ID and tag ID are saved in a column of maximum length 255 respectively.
     *
    ...
    private function createTagsTable(): void
    {
        $sql = match ($driver = $this->getDriver()) {
            ...
            'mysql' => "CREATE TABLE $this->tagsTable ($this->idCol VARBINARY(255) NOT NULL, $this->tagCol VARBINARY(255) NOT NULL, PRIMARY KEY($this->idCol, $this->tagCol), INDEX $this->tagIdxName($this->tagCol)) ENGINE = InnoDB",
        ...
        // run the statements one by one: Oracle rejects ";"-separated batches,
        // and pdo_sqlite silently ignores everything after the first statement
    ...
    {
        $stmt = $this->prepareStatementWithFallback("DELETE FROM $this->tagsTable WHERE $this->idCol NOT IN (SELECT $this->idCol FROM $this->table)", function () {
            $this->createTagsTable();
        });
        $stmt->execute();
