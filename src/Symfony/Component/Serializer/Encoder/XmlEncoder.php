    ...
    private function buildXml($parentNode, $data)
    {
        ...
        if (is_array($data) || $data instanceof \Traversable) {
            foreach ($data as $key => $value) {
                ...
                if (0 === strpos($key, "@") && is_scalar($value) && $this->isElementNameValid($attributeName = substr($key, 1))) {
                    $parentNode->setAttribute($attributeName, $value);
                } elseif ($key === '#') {
                    $append = $this->selectNodeType($parentNode, $value);
                } elseif (is_array($value) && false === is_numeric($key)) {
                    /**
                    ...
                    if (ctype_digit(implode('', array_keys($value)))) {
                        ...
                        $append = $this->appendNode($parentNode, $value, $key);
                    }
                } elseif (is_numeric($key) || !$this->isElementNameValid($key)) {
                    $append = $this->appendNode($parentNode, $value, "item", $key);
                } else {
                    $append = $this->appendNode($parentNode, $value, $key);
                }
            }
