...
class OpenSslKmsFactoryTest extends TestCase
{
    ...
    public function testCreateOnUnsupportedSchemeThrows()
    {
        ...
        (new OpenSslKmsFactory())->create(Dsn::fromString('hashicorp-vault-transit://x@host'));
    }
    ...
    public function testInMemoryDsnRejectsAKeyWhosePlusBecameASpace()
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Key "app" in the DSN contains whitespace: inline keys must be base64url-encoded or percent-encoded ("+" as "%2B").');
        (new OpenSslKmsFactory())->create(Dsn::fromString('openssl://?keys[app]='.base64_encode(str_repeat("\xfb\xef\xbe", 11))));
    }

    public function testInMemoryDsnAcceptsAPercentEncodedStandardBase64Key()
    {
        $key = str_repeat("\xfb\xef\xbe", 10).'xx';
        $dsn = 'openssl://?keys[app]='.rawurlencode(base64_encode($key));
