    ...
    public function getUserBadgeFrom(string $accessToken): UserBadge
    {
        try {
            ...
            if (empty($claims['sub']) && empty($claims['username'])) {
                throw new MissingClaimException(sprintf('"%s" and "%s" claims not found on the authorization server response. At least one is required.', 'sub', 'username'));
            }
            if (!($claims['active'] ?? false)) {
                throw new MissingClaimException(sprintf('The claim "active" was not found on the authorization server response or is set to false.', 'active'));
            }
