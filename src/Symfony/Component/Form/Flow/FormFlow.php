...
class FormFlow extends Form implements FormFlowInterface
{
    ...
    private function moveBackTo(string $step): void
    {
        ...
        $stepNode = $this->cursor->getStepNode($step);

        if ($stepNode->isGroupOrSkipped($data)) {
            throw new RuntimeException(\sprintf('Cannot move back to step "%s" because it is %s.', $step, $stepNode->isGroup() ? 'a group' : 'a skipped step'));
        }
