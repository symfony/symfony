...
namespace Symfony\Bundle\FoundationBundle\Tests\DependencyInjection;
...
class WebExtensionTest extends TestCase
{
    public function testConfigLoad()
    {
        ...
        $this->assertEquals('Symfony\\Bundle\\FoundationBundle\\RequestListener', $configuration->getParameter('request_listener.class'), '->webLoad() loads the web.xml file if not already loaded');

        $configuration = new BuilderConfiguration();
        $loader = $this->getWebExtension();

        $configuration = $loader->configLoad(array('profiler' => true), $configuration);
        $this->assertEquals('Symfony\\Bundle\\FoundationBundle\\Profiler', $configuration->getParameter('profiler.class'), '->configLoad() loads the collectors.xml file if not already loaded');
    ...
    public function testTemplatingLoad()
    {
        ...
        $this->assertEquals('Symfony\\Bundle\\FoundationBundle\\Templating\\Engine', $configuration->getParameter('templating.engine.class'), '->templatingLoad() loads the templating.xml file if not already loaded');
    }
