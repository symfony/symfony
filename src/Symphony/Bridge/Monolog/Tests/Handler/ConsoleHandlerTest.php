...
namespace Symphony\Bridge\Monolog\Tests\Handler;

use Monolog\Logger;
use PHPUnit\Framework\TestCase;
use Symphony\Bridge\Monolog\Handler\ConsoleHandler;
...
class ConsoleHandlerTest extends TestCase
{
    ...
    {
        $output = $this->getMockBuilder('Symphony\Component\Console\Output\OutputInterface')->getMock();
        $output
        ...
        $realOutput = $this->getMockBuilder('Symphony\Component\Console\Output\Output')->setMethods(array('doWrite'))->getMock();
    ...
    {
        $handler = new ConsoleHandler();
        $this->assertInstanceOf('Symphony\Bridge\Monolog\Formatter\ConsoleFormatter', $handler->getFormatter(),
    ...
    public function testLogsFromListeners()
    {
        ...
        $event = new ConsoleCommandEvent(new Command('foo'), $this->getMockBuilder('Symphony\Component\Console\Input\InputInterface')->getMock(), $output);
        $dispatcher->dispatch(ConsoleEvents::COMMAND, $event);
        $this->assertContains('Before command message.', $out = $output->fetch());
        $this->assertContains('After command message.', $out);

        $event = new ConsoleTerminateEvent(new Command('foo'), $this->getMockBuilder('Symphony\Component\Console\Input\InputInterface')->getMock(), $output, 0);
