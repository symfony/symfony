 ...
 * removed `acl` configuration key and related services, use symphony/acl-bundle instead
 ...
 * Added new `security.helper` service that is an instance of `Symphony\Component\Security\Core\Security`
 ...
 * Added the `SecurityUserValueResolver` to inject the security users in actions via
   `Symphony\Component\Security\Core\User\UserInterface` in the method signature.
 ...
 * added new events: `security.authentication.success` and `security.authentication.failure`
   ...
   events are defined in new event class: `Symphony\Component\Security\Core\AuthenticationEvents`.
