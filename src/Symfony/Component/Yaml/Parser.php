            ...
            if (self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
                ...
                if (!(Yaml::PARSE_KEYS_AS_STRINGS & $flags) && !is_string($key)) {
                    ...
                    @trigger_error(sprintf(
                        'Implicit casting of %s to string on line %d is deprecated since version 3.3 and will throw \Symfony\Component\Yaml\Exception\ParseException in 4.0. Pass the PARSE_KEYS_AS_STRING flag to explicitly enable the type casts.',
                        $keyType,
