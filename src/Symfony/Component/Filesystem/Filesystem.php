...
class Filesystem
{
    ...
    public function tempNam($dir, $prefix) {
        ...
        $dir = $this->getHierachy($dir);
        // Loop until we create a valid temp file or have reached $limit attempts
        for ($i = 0; $i < $limit; $i++) {
            // Create temp file in filesystem. We do this to use the file name generated by the OS.
            $tmpFile = tempnam($dir, $prefix);
            // If scheme is file
            if ('file' === $scheme) {
               // If the directory is the same
                if (realpath($dir) === dirname($tmpFile)) {
                    // Add the scheme back on
                    $tmpFile = $scheme.$schemeSeparator.$dir.DIRECTORY_SEPARATOR.basename($tmpFile);
                }
            }
            // If scheme exists and is valid
                ...
                $tmpFile = $scheme.$schemeSeparator.$dir.'/'.basename($tmpFile);
                // If the file already exists set to FALSE and try again
                if (file_exists($tmpFile)) {
                    $tmpFile = false;
                    continue;
                }
                // Create the file, if unsuccessful then stream is not writable so set to FALSE
                // We use file_put_contents instead of touch as it support stream wrappers
    ...
    public function dumpFile($filename, $content, $mode = 0666)
    {
        ...
        if (null !== $mode) {
            $scheme = $this->getScheme($filename);
            if (false !==  $scheme && 'file' !== $scheme) {
                trigger_error('Support for modifying file permissions on streams is not implemented.', E_USER_WARNING);
                return;
            }
            if (func_num_args() > 2) {
            ...
            // Remove 'file://' scheme as PHP 5.3 doesn't support schemes in chmod
            $this->chmod($this->getHierachy($filename), $mode);
        }
    }
    ...
    /**
     * Gets the hierachy part of a filename (e.g. file:///tmp -> /tmp).
     * @param string $filename The filename to be parsed.
     *
     * @return string The filename hierachy.
     */
    private function getHierachy($filename)
    {
        $schemeSeparator = '://';
        $components = explode($schemeSeparator, $filename, 2);
