...
class AuthenticatedVoterTest extends TestCase
{
    ...
    public function testRecentlyAuthenticatedVoteReasons()
    {
        ...
        // a remembered user is not stale, they never authenticated in this session
        $voter->vote($this->getToken('remembered'), null, ['IS_AUTHENTICATED_RECENTLY'], $remembered = new Vote());
        $this->assertSame(['The user is not fully authenticated.'], $remembered->reasons);
    }

    #[DataProvider('provideDenialReasons')]
    public function testTheDenialReasonNamesTheAttributeThatFailed(string $authenticated, array $attributes, array $expectedReasons)
    {
        $voter = new AuthenticatedVoter(new AuthenticationTrustResolver(), 900);
    ...
    public static function provideDenialReasons()
    {
        ...
        yield 'each failing attribute is named once' => ['remembered', ['IS_AUTHENTICATED_FULLY', 'IS_IMPERSONATOR'], ['The user is not fully authenticated.', 'The user is not impersonating another user.']];
        yield 'a repeated reason is not repeated' => ['remembered', ['IS_AUTHENTICATED_FULLY', 'IS_AUTHENTICATED_RECENTLY'], ['The user is not fully authenticated.']];
    }
