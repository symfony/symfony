...
class PhpDumper extends Dumper
{
    ...
    public function dump(array $options = array())
    {
        ...
        $code =
            $this->startFile($options['namespace']).
        ...
        ;
        if ($this->asFiles) {
            $fileStart = $this->startFile($options['namespace']).
                "\n// This file has been auto-generated by the Symfony Dependency Injection Component for internal use.\n\n";
            $code = array($options['class'].'.php' => $code);
            foreach ($this->generateServiceFiles() as $file => $c) {
                $code[$file] = $fileStart.$c;
            }
        }
        $this->targetDirRegex = null;
    ...
    private function addService($id, Definition $definition, &$file = null)
    {
        ...
        $file = sprintf('%s/%s.php', $this->class, $methodName);
        if ($asFile) {
...
EOF;
        ...
        if ($this->getProxyDumper()->isProxyCandidate($definition)) {
            $factoryCode = $asFile ? "\$this->requireInScope(__DIR__.''.'/%s.php', false)" : '$this->%s(false)';
        ...
        if ($this->asFiles) {
            $code = str_replace("\$this->requireInScope(__DIR__.''.'", "\$this->requireInScope(__DIR__.'".($asFile ? '' : '/'.$this->class), $code);
        }
     ...
     * @return string
     */
    private function addServiceMethods()
    {
...
EOF;
    ...
    private function generateServiceFiles()
    {
        ...
        ksort($definitions);
        foreach ($definitions as $id => $definition) {
            if (!$definition->isSynthetic() && $definition->isShared() && !$this->isTrivialInstance($definition)) {
    ...
    /**
     * Adds the PHP file headers.
     *
     * @param string $namespace The class namespace
     *
     * @return string
     */
    private function startFile($namespace)
    {
    ...
    private function addFileMap()
    {
        ...
        foreach ($definitions as $id => $definition) {
            if (!$definition->isSynthetic() && $definition->isPublic() && $definition->isShared()) {
                $code .= sprintf("            %s => __DIR__.'/%s/%s.php',\n", $this->export($id), $this->class, $this->generateMethodName($id));
            }
        }
...
EOF;
    ...
    private function getServiceCall($id, Reference $reference = null)
    {
        ...
        if ($this->container->hasDefinition($id)) {
            ...
            if ($this->isTrivialInstance($definition)) {
                ...
                $code = sprintf("\$this->requireInScope(__DIR__.''.'/%s.php')", $this->generateMethodName($id));
            } else {
    ...
    {
        $targetDir0 = $this->asFiles ? '$this->targetDirs[] =' : '';

        return null === $this->targetDirRegex ? '' : <<<EOF

        \$dir ={$targetDir0} __DIR__;
